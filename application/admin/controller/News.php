...
class News extends Controller
{
    ...
    {
        if ($this->request->param("head")) {
            $map['head'] = ["like", "%" . $this->request->param("head") . "%"];
        }
    ...
    public function add()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            // 添加
                ...
                $newd=$newdModel->field('id,titles')->where([
                'isdelete'=>0,
                'status'=>1
                ])->select();
                // return json($newd);
            return $this->view->fetch(isset($this->template) ? $this->template : 'edit',['newd'=>$newd]);
        }
    }
