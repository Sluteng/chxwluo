...
class Baksql extends Controller {
    private $config=[];
    private $handler;
    ...
    public function __construct($config) {
        ...
        $config['path']=ROOT_PATH .'data/'; //默认目录
        $config["sqlbakname"]=$dbname.'_'.date("YmdHis",time()).".sql"; //默认保存文件
        $this->config = $config;
    ...
    private function writeToFile($tables = array(), $ddl = array(), $data = array())
    {
        ...
        {
            if($table=='tp_sql' || $table=='tp_web_log_all' || $table=='tp_login_log' || $table=='tp_login_num' || $table=='tp_number'){
            $str .= "-- ----------------------------\r\n";
        ...
        $sql['title'] = $this->config['sqlbakname'];
        $sql['url'] = urlencode($this->config['path'].$this->config['sqlbakname']);
        $sql['create_time'] = time();
        $sql['update_time'] = time();
        $validate = validate('Sql');
        if (!$validate->batch()->check($sql)) {
          $error = $validate->getError();
          $this->error(implode(',',$error)); //返回上一页
          }
          $center = model('Sql');
          $center->data($sql,true);
    ...
    public function restore($filename = '')
    {
        ...
        else
        {
            ...
            try
            {
                ...
                // 还原后删除文件和数据
                // if (@unlink($path)) {
                //     $sqlModel=Loader::model('Sql');
                //     $sqlResult=$sqlModel->where([
                //             'title'=>$filename,
                //         ])->delete(); 
                // }
                // unlink($path);     //还原后删除此文件
                $this->success('还原成功!花费时间'.round(microtime(true) - $this->begin,2) . 'ms');
                // return file_put_contents($this->config['path'].$this->config['sqlbakname'], $str) ? '备份成功!花费时间' . round(microtime(true) - $this->begin,2) . 'ms' : '备份失败!';
